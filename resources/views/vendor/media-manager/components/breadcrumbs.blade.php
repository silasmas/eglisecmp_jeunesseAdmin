...
<div class="flex-1 min-w-0 flex flex-wrap sm:flex-nowrap items-center gap-1 text-sm font-medium text-gray-500 dark:text-gray-400">
    ...
    >
        <x-filament::icon icon="heroicon-s-home" class="w-5 h-5 text-gray-400 dark:text-gray-500" />
    </button>

    @if($count > 0)
        @if($hasHidden)
            <div x-data="{ dropdownOpen: false }" class="relative flex items-center shrink-0">
                <x-filament::icon icon="heroicon-m-chevron-right" class="w-4 h-4 mx-1 text-gray-300 dark:text-gray-600 flex-shrink-0" />
